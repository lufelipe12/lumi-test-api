...
export const pdfTextToBill = (pdfText: string): CreateBillDto => {
  ...
  const regexElectricEnergy = /Energia Elétrica.*?(\d+)\s+([\d,]+).*?([\d,]+)/s;
  ...
  const electricityValue = matchElectricEnergy ? matchElectricEnergy[3] : null;
